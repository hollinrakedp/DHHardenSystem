function TLS1Client {
    ...
    Write-LogEntry -Tee:$Tee -LogMessage "HardenSystem: Mitigation: TLS 1.0 Client - Begin"
    ...
    foreach ($KeyProperty in $KeyProperties) {
        ...
        if ($PSCmdlet.ShouldProcess("$RegPath::$($KeyProperty.Name)", 'Set registry value')) {
            Write-LogEntry -Tee:$Tee -LogMessage "HardenSystem: Mitigation: TLS 1.0 Client - Setting $($KeyProperty.Name)=$($KeyProperty.Value)"
            New-ItemProperty @ItemProperty | Out-Null
        }
    }

    Write-LogEntry -Tee:$Tee -LogMessage "HardenSystem: Mitigation: TLS 1.0 Client - Complete"
}